...
can get down to the kernel level, instead opting, for example for intercepting system calls using \texttt{DL\_PRELOAD} \cite{HPCIODarshan}, this approach leaves out important information about whats happening at levels closer to the operating system.\\
...
One technology that has gained significant traction is the extended Berkeley Packet Filter (eBPF).
...
With this in mind, our objective is to address the aforementioned challenges by developing a tool capable of tracing different kernel levels to collect in-kernel statistics.
This data will enable us to correlate and characterize patterns during deep learning model training.

Existing tools, such as those developed using bpftrace, a high-level language that abstracts some of the underlying eBPF logic, can be injected into various parts of the kernel to trace diverse targets, such as CPU, memory, disk, the virtual file system, and network.
Our goal is to leverage and modify these tools to accommodate the specific needs of deep learning training workflows.

Besides that, it is crucial to evaluate the overhead and limitations of these tools to determine which are most suitable for our study. Understanding the appropriate layers and points within the kernel where they should be applied will be an essential part of this process.
