...
% tentar fazer isto por grupos, ainda tou a tentar ganhar forma nisto :<
%We group the related work into three categories:
%\begin{itemize}
%    \item Characterization of Machine Learning workloads using approaches that do not dive in in-kernel metric collection;
%    \item Work that uses eBPF for observation of interactions between the non machine learning related applications and the kernel;
%    \item Similar work that leverages eBPF in order to characterize some aspect of kernel related statistics.
%\end{itemize}
