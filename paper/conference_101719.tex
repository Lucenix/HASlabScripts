...
There are some studies that uses eBPF's capability to provide kernel-level information in order to understand patterns.

Leverage eBPF to monitor distributed deep learning training, focusing on optimizing resource allocation and traffic in multi-tenant environments \cite{eBPFDLNetwork}.

eBPF has been utilized to characterize machine and deep learning workloads, focusing on storage and I/O efficiency in training, 
...
We analysed the write and read patterns to pipes of the Alexnet model, we found aproximately twice as much reads from pipes as there were writes, upon lookin into the patter we noticed that
each time a pipes as read from there was an initial read of 4 bytes followed by a subsequent read of more bytes.
...
\subsubsection{Pattern of block I/O acceses}
During the first epoch we found a large number of initial reads followed by drop up until the moment the second epoch begins, after which there is a uniform pattern of read's.
This pattern was consistent with changes to the number of epochs, but the length of the initial burst of read's increased as the number of data loading workers decresed (no workers??).
This pattern also mantained when the operating systems caches were dropped in between epochs 
