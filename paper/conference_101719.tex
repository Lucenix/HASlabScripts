...
\section{Background}
    ...
    \textit{Deep learning} is a subset of machine learning algorithms based on neural networks with many layers. Training is done by passing all data
    through the entire network to calculate a loss, this is called the forward pass, the network parameters are then updated using the calculated loss,
    ...
    the model parameters among all the workers, each of the individualy processes a subset of the data, the loss is then averaged and all the workers update their
    parameters with the same loss, ensuring consistency among all workers.
